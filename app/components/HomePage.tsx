...
type WorkCase = { title: string; context: string; delivery: string; outcome: string };
type Copy = {
  lang: string;
  nav: [string, string][];
...
const content: Record<Language, Copy> = {
  en: {
    ...
    heroSignals: [["Market signal", "ICP, trigger, pain and timing before outreach."], ["Qualified pipeline", "Discovery, urgency, stakeholder map and next step."], ["CRM discipline", "Fields, stages, activities and ownership with management value."], ["Forecast rhythm", "Operating cadence to read risk and improve decisions."]],
    ...
    work: [
      { title: "Sales Enablement for IT Outsourcing", context: "Complex service conversations needed clearer qualification and business narrative.", delivery: "ICP, personas, discovery flow, objection handling, proposal logic and pipeline action plan.", outcome: "More consistent commercial conversations and sharper next-step discipline." },
      { title: "Pipedrive CRM Governance", context: "Pipeline visibility depended on cleaner fields, stages and operating rhythm.", delivery: "Pipeline architecture, field groups, required fields, loss reasons, activities and forecast logic.", outcome: "Better management signal for ownership, risk reading and decision cadence." },
      { title: "Outbound Operating System", context: "Prospecting needed stronger account prioritization and a reason to engage.", delivery: "Account tiers, buying triggers, contact mapping, messaging, cadence and follow-up structure.", outcome: "Outbound execution with clearer fit, timing and conversation quality." },
      { title: "Commercial Materials & Playbooks", context: "Sales assets needed to translate scattered context into usable execution.", delivery: "Executive-grade materials, talk tracks, playbook sections and reusable decision support.", outcome: "Teams get clearer language for discovery, proposals and stakeholder alignment." },
    ],
    callTitle: "What happens in a first call?",
    callText: "A short conversation to understand context, commercial challenges and whether there is a practical next step.",
    callSteps: [["01", "Context"], ["02", "Commercial challenge"], ["03", "Next step"]],
  ...
  pt: {
    ...
    heroSignals: [["Sinal de mercado", "ICP, gatilho, dor e timing antes da abordagem."], ["Pipeline qualificado", "Discovery, urgência, mapa de decisores e próximo passo."], ["Disciplina de CRM", "Campos, etapas, atividades e dono com valor de gestão."], ["Ritmo de forecast", "Cadência operacional para ler risco e melhorar decisões."]],
    ...
    work: [
      { title: "Sales Enablement para Outsourcing de TI", context: "Conversas de serviço complexo precisavam de qualificação e narrativa de negócio mais claras.", delivery: "ICP, personas, fluxo de discovery, objeções, lógica de proposta e plano de ação de pipeline.", outcome: "Conversas comerciais mais consistentes e mais disciplina nos próximos passos." },
      { title: "Governança de CRM no Pipedrive", context: "A visibilidade de pipeline dependia de campos, etapas e rotina operacional mais limpos.", delivery: "Arquitetura de pipeline, grupos de campos, campos obrigatórios, motivos de perda, atividades e forecast.", outcome: "Melhor sinal de gestão para dono, risco e cadência de decisão." },
      { title: "Sistema Operacional Outbound", context: "A prospecção precisava de priorização de contas e uma razão mais clara para abordar.", delivery: "Tiers de contas, gatilhos de compra, mapeamento de contatos, mensagens, cadência e follow-up.", outcome: "Execução outbound com mais clareza de fit, timing e qualidade de conversa." },
      { title: "Materiais Comerciais & Playbooks", context: "Ativos de venda precisavam transformar contexto disperso em execução utilizável.", delivery: "Materiais executivos, talk tracks, seções de playbook e apoio reutilizável à decisão.", outcome: "Times ganham linguagem mais clara para discovery, proposta e alinhamento com stakeholders." },
    ],
    callTitle: "Como funciona a primeira chamada?",
    callText: "Uma conversa curta para entender contexto, desafio comercial e se existe um próximo passo prático.",
    callSteps: [["01", "Contexto"], ["02", "Desafio comercial"], ["03", "Próximo passo"]],
...
function LogoLockup({ subtitle }: { subtitle: string }) {
  return <div className="inline-flex max-w-full items-center gap-3 sm:gap-4"><BrandMark className="h-9 w-9 shrink-0 sm:h-10 sm:w-10" /><div className="min-w-max leading-none"><div className="whitespace-nowrap text-[13px] font-semibold tracking-[0.22em] text-[#F7F5F0] sm:text-sm sm:tracking-[0.26em] md:text-base md:tracking-[0.34em]">RICARDO ZULK</div><div className="mt-2 whitespace-nowrap text-[9px] font-medium tracking-[0.18em] text-[#57a6b7] sm:text-[10px] sm:tracking-[0.22em] md:text-xs md:tracking-[0.32em]">{subtitle}</div></div></div>;
}
...
function LanguageSwitch({ language, mobile = false }: { language: Language; mobile?: boolean }) {
  ...
  return <div className={base} aria-label="Language"><Link href="/" aria-current={language === "en" ? "page" : undefined} className={`${item} ${language === "en" ? active : ""}`}>EN</Link><Link href="/pt" aria-current={language === "pt" ? "page" : undefined} className={`${item} ${language === "pt" ? active : ""}`}>PT</Link></div>;
}
...
export default function HomePage({ language }: { language: Language }) {
  ...
  return (
    <main lang={c.lang} className="min-h-screen bg-[#1F1F1F] text-[#F7F5F0] antialiased selection:bg-[#0F4C5C] selection:text-[#F7F5F0]">
      <a href="#top" className="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-50 focus:bg-[#F7F5F0] focus:px-4 focus:py-3 focus:text-sm focus:font-semibold focus:text-[#1F1F1F]">
        {language === "pt" ? "Pular para o conteúdo" : "Skip to content"}
      </a>
      <section className="relative overflow-hidden border-b hairline"><div className="absolute inset-0 bg-[radial-gradient(circle_at_76%_26%,rgba(15,76,92,0.24),transparent_32%),radial-gradient(circle_at_12%_0%,rgba(255,255,255,0.06),transparent_24%)]" /><div className="absolute right-[-7%] top-[11%] hidden h-[540px] w-[540px] opacity-45 lg:block"><BrandMark className="h-full w-full" subtle /></div><header className="relative z-20 mx-auto flex max-w-7xl items-center justify-between px-4 py-6 sm:px-6 sm:py-7 lg:px-10"><a href="#top" aria-label="Ricardo Zulk home"><LogoLockup subtitle={c.brandSubtitle} /></a><nav aria-label={language === "pt" ? "Navegação principal" : "Main navigation"} className="hidden items-center gap-10 text-[11px] font-medium uppercase tracking-[0.24em] text-[#D8D8D8]/75 lg:flex">{c.nav.map(([label, href]) => <a key={label} className="transition hover:text-[#F7F5F0]" href={href}>{label}</a>)}</nav><div className="flex items-center gap-3"><LanguageSwitch language={language} /><a href={calendlyUrl} target="_blank" rel="noopener noreferrer" onClick={() => trackEvent("click_calendly", "header")} className="hidden items-center gap-3 whitespace-nowrap border border-[#0F4C5C]/70 bg-[#0F4C5C]/80 px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.20em] text-[#F7F5F0] transition hover:bg-[#0F4C5C] md:flex">{c.scheduleCall} <CalendarCheck size={15} /></a><button type="button" onClick={() => setIsMenuOpen((value) => !value)} className="inline-flex h-11 w-11 items-center justify-center border hairline text-[#F7F5F0] lg:hidden" aria-label={isMenuOpen ? "Close navigation" : "Open navigation"} aria-expanded={isMenuOpen} aria-controls="mobile-navigation">{isMenuOpen ? <X size={18} /> : <Menu size={18} />}</button></div></header>{isMenuOpen && <div id="mobile-navigation" className="relative z-20 mx-4 mb-6 border hairline bg-[#1F1F1F]/95 p-5 backdrop-blur sm:mx-6 lg:hidden"><LanguageSwitch language={language} mobile /><nav aria-label={language === "pt" ? "Navegação mobile" : "Mobile navigation"} className="grid gap-4 text-[12px] font-semibold uppercase tracking-[0.22em] text-[#D8D8D8]/80">{c.nav.map(([label, href]) => <a key={label} href={href} onClick={() => setIsMenuOpen(false)} className="border-b hairline pb-4 last:border-b-0 last:pb-0">{label}</a>)}</nav></div>}<div id="top" className="relative z-10 mx-auto grid max-w-7xl gap-14 px-4 pb-14 pt-10 sm:px-6 lg:grid-cols-[1fr_0.78fr] lg:px-10 lg:pb-20 lg:pt-20"><div className="animate-enter-up"><p className="mb-8 text-[11px] font-semibold uppercase tracking-[0.28em] text-[#57a6b7]">{c.heroEyebrow}</p><h1 className="max-w-5xl text-[44px] font-light leading-[1.04] tracking-[-0.06em] text-[#F7F5F0] sm:text-5xl md:text-7xl lg:text-[84px]">{c.heroBefore} <span className="italic tracking-[-0.075em] text-[#F7F5F0]/90">{c.heroHighlight}</span> {c.heroAfter}</h1><p className="mt-8 max-w-2xl text-base leading-8 text-[#D8D8D8]/78 md:text-lg">{c.heroText}</p><div className="mt-10 flex flex-col gap-4 sm:flex-row sm:flex-wrap"><a href={calendlyUrl} target="_blank" rel="noopener noreferrer" onClick={() => trackEvent("click_calendly", "hero")} className="inline-flex items-center justify-center gap-3 whitespace-nowrap bg-[#0F4C5C] px-7 py-4 text-[11px] font-semibold uppercase tracking-[0.20em] text-[#F7F5F0] transition hover:-translate-y-0.5 hover:bg-[#126177]">{c.scheduleCall} <CalendarCheck size={16} /></a><a href={mailto} onClick={() => trackEvent("click_email", "hero")} className="inline-flex items-center justify-center gap-3 whitespace-nowrap border border-[#D8D8D8]/20 px-7 py-4 text-[11px] font-semibold uppercase tracking-[0.20em] text-[#F7F5F0] transition hover:-translate-y-0.5 hover:border-[#F7F5F0]/45">{c.primaryCta} <Mail size={16} /></a><a href="#work" className="inline-flex items-center justify-center gap-3 whitespace-nowrap border border-[#D8D8D8]/20 px-7 py-4 text-[11px] font-semibold uppercase tracking-[0.20em] text-[#F7F5F0] transition hover:-translate-y-0.5 hover:border-[#F7F5F0]/45">{c.secondaryCta}</a></div></div><aside className="animate-enter-side self-end border hairline bg-[#F7F5F0]/[0.035] p-6 backdrop-blur soft-glow"><div className="mb-12 flex items-start justify-between gap-6"><div><p className="text-[11px] font-semibold uppercase tracking-[0.26em] text-[#57a6b7]">{c.profileTitle}</p><p className="mt-3 max-w-xs text-sm leading-6 text-[#D8D8D8]/65">{c.profileText}</p></div><BrandMark className="h-20 w-20 shrink-0 sm:h-24 sm:w-24" /></div><div className="grid gap-6">{c.metrics.map(([value, label, detail]) => <div key={label} className="border-t hairline pt-5"><div className="text-2xl font-light tracking-[-0.04em] text-[#F7F5F0] md:text-3xl">{value}</div><div className="mt-2 text-[11px] font-semibold uppercase tracking-[0.20em] text-[#57a6b7]">{label}</div><div className="mt-2 text-sm leading-6 text-[#D8D8D8]/65">{detail}</div></div>)}</div></aside></div><div className="relative z-10 mx-auto max-w-7xl px-4 pb-16 sm:px-6 lg:px-10 lg:pb-24"><div className="grid gap-px overflow-hidden border hairline bg-[#F7F5F0]/10 md:grid-cols-4">{c.heroSignals.map(([label, detail]) => <div key={label} className="bg-[#1F1F1F]/92 p-5 sm:p-6"><p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[#57a6b7]">{label}</p><p className="mt-3 text-sm leading-6 text-[#D8D8D8]/66">{detail}</p></div>)}</div></div></section>
      ...
      <section id="work" className="px-6 py-24 lg:px-10"><div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.62fr_1fr]"><div><Eyebrow>{language === "pt" ? "Projetos selecionados" : "Selected work"}</Eyebrow><h2 className="text-4xl font-light leading-tight tracking-[-0.05em] md:text-5xl">{c.workTitle}</h2></div><div className="grid gap-5">{c.work.map((project) => <article key={project.title} className="group border-b hairline py-7"><div className="flex gap-5"><ArrowUpRight className="mt-1 shrink-0 text-[#57a6b7] transition group-hover:translate-x-1 group-hover:-translate-y-1" size={18} /><div><h3 className="text-sm font-semibold uppercase tracking-[0.18em] text-[#F7F5F0]">{project.title}</h3><div className="mt-5 grid gap-4 text-sm leading-7 text-[#D8D8D8]/72 md:grid-cols-3"><div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#57a6b7]">{language === "pt" ? "Contexto" : "Context"}</p><p className="mt-2">{project.context}</p></div><div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#57a6b7]">{language === "pt" ? "Entrega" : "Delivery"}</p><p className="mt-2">{project.delivery}</p></div><div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#57a6b7]">{language === "pt" ? "Sinal de valor" : "Value signal"}</p><p className="mt-2">{project.outcome}</p></div></div></div></div></article>)}</div></div></section>
      ...
      <section className="border-t hairline px-6 py-20 lg:px-10"><div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.6fr_1fr] lg:items-center"><div><Eyebrow>{language === "pt" ? "Primeira chamada" : "First call"}</Eyebrow><h2 className="text-4xl font-light leading-tight tracking-[-0.05em] md:text-5xl">{c.callTitle}</h2><p className="mt-6 max-w-2xl text-base leading-8 text-[#D8D8D8]/72">{c.callText}</p></div><div className="grid gap-px overflow-hidden border hairline bg-[#F7F5F0]/10 md:grid-cols-3">{c.callSteps.map(([number, label]) => <div key={label} className="bg-[#1F1F1F] p-7"><div className="mb-8 text-3xl font-light text-[#F7F5F0]/55">{number}</div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#F7F5F0]">{label}</p></div>)}</div></div></section>
      <section id="contact" className="px-6 pb-10 pt-10 lg:px-10"><div className="mx-auto max-w-7xl border hairline bg-[#0F4C5C]/70 p-8 md:p-12"><div className="grid gap-10 md:grid-cols-[0.9fr_1fr_0.8fr] md:items-center"><h2 className="text-3xl font-light leading-tight tracking-[-0.05em] md:text-5xl">{c.finalTitle}</h2><p className="text-base leading-8 text-[#F7F5F0]/78">{c.finalText}</p><div className="flex flex-col gap-3 sm:flex-row md:flex-col xl:flex-row"><a href={mailto} onClick={() => trackEvent("click_email", "contact")} className="inline-flex items-center justify-center gap-3 whitespace-nowrap border border-[#F7F5F0]/20 bg-[#F7F5F0] px-6 py-4 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#1F1F1F] transition hover:-translate-y-0.5">{c.contact} <Mail size={15} /></a><a href={calendlyUrl} target="_blank" rel="noopener noreferrer" onClick={() => trackEvent("click_calendly", "contact")} className="inline-flex items-center justify-center gap-3 whitespace-nowrap border border-[#F7F5F0]/24 px-6 py-4 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#F7F5F0] transition hover:-translate-y-0.5 hover:border-[#F7F5F0]/55">{c.scheduleCall} <CalendarCheck size={15} /></a></div></div></div><footer className="mx-auto mt-10 flex max-w-7xl flex-col gap-8 border-t hairline py-8 md:flex-row md:items-center md:justify-between"><LogoLockup subtitle={c.brandSubtitle} /><div className="flex flex-wrap items-center gap-5 text-sm text-[#D8D8D8]/65"><span className="inline-flex items-center gap-2"><MapPin size={15} /> {c.location}</span><a className="inline-flex items-center gap-2 transition hover:text-[#F7F5F0]" href={mailto} onClick={() => trackEvent("click_email", "footer")}><Mail size={15} /> Email</a><a className="inline-flex items-center gap-2 transition hover:text-[#F7F5F0]" href={linkedinUrl} target="_blank" rel="noopener noreferrer" onClick={() => trackEvent("click_linkedin", "footer")}><Linkedin size={15} /> LinkedIn</a><Link className="inline-flex items-center gap-2 transition hover:text-[#F7F5F0]" href="/cv" onClick={() => trackEvent("click_cv", "footer")}><Download size={15} /> CV</Link><Link className="inline-flex items-center gap-2 transition hover:text-[#F7F5F0]" href={language === "pt" ? "/" : "/pt"}>{language === "pt" ? "EN" : "PT"}</Link></div></footer></section>
    </main>
  );
}
